...
let dx, dy, dt, speed;
function mouseMoved() {
  currentFrame = { x: mouseX, y: mouseY, time: millis() };
  if (prevFrame) {
    dx = currentFrame.x - prevFrame.x;
    dy = currentFrame.y - prevFrame.y;
    dt = (currentFrame.time - prevFrame.time) / 1000; // convert to seconds
    speed = dist(0, 0, dx, dy) / dt;
    console.log("Mouse speed: " + speed + " pixels/second");
  }
...
class Splash extends Agent {
  ...
  show() {
    ...
    if (this.shapeType == 0) {
      ...
      noStroke();
      fill(this.clr);
      if (random() < 0.8) rect(0, 0, width * 0.01 * (1 - this.amount));
      pop();
    }
